...
class Upload {
	...
	public static function file($file, $moveTo, $maxSize, $allowedExtensions = array(), $args = array()) {
		...
		if ($file['error'] > 0) {
			...
			\Components::Alert('danger', $message);
			return false;
		}
		...
		if ($file['size'] > ($maxSize * 1024)) {
			\Components::Alert('danger', 'Le fichier fait ' . Sanitize::readableFileSize($file['size']) . ' alors que la taille maximum autorisée est de ' . $maxSize . 'ko !');
			return false;
		}
		...
		if (!empty($allowedExtensions)) {
			...
			if (!in_array($extension, $allowedExtensions)) {
				\Components::Alert('danger', 'Les fichiers <code>.' . $extension . '</code> ne sont pas autorisés !');
				return false;
			}
		}
			...
			if (isset($args['resize']) and in_array(pathinfo($file['name'])['extension'], unserialize(\Settings::ALLOWED_IMAGES_EXT))) {
				...
				if (isset ($resizeArgs['width']) or isset($resizeArgs['height'])) {
					...
					try {
						...
						\Components::Alert('danger', 'Erreur de redimensionnement : ' . $e->getMessage());
						return false;
					}
				} else {
		...
		if (isset ($img)) {
			try {
				...
				\Components::Alert('danger', 'impossible de sauvegarder l\'image : ' . $e->getMessage());
				return false;
			}
		} else {
			if (!move_uploaded_file($file['tmp_name'], $moveTo . $name)) {
				\Components::Alert('danger', 'impossible de sauvegarder le fichier !');
				return false;
			}
		}
		return $name;
