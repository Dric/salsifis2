...
class Ftp {
	...
	function connect(){
		switch( $this->connectionType )
		{
			case 'ftp':
				...
				if (!$connection){
					\Components::Alert('danger', 'Impossible de se connecter au serveur distant <code>'.$this->host.'</code>');
					return false;
				}
				$login = ftp_login($connection, $this->username, $this->password);
				if (!$login){
					\Components::Alert('danger', 'Les identifiants utilisés pour la connexion FTP sont incorrects');
					return false;
				}
			...
			case 'sftp':
				...
				switch( $this->sshType ){
					...
					default:
						\Components::Alert('danger', 'No ssh method defined, please define one in: $ftp_sftp->sshType');
						return false;
				}
				$this->connection = $connection;
				return true;
	...
	public function connectionCheck(){
		if( $this->connection === false){
			\Components::Alert('danger', 'La connexion au serveur distant n\'est pas établie !');
			return false;
		}
		return true;
	}
	...
	public function put($fileDest, $fileFrom){
		...
		if ($this->verboseMode){
			if ($put){
				...
				\Components::Alert('danger', 'Le fichier <code>'.$fileFrom.'</code> n\'a pas pu être envoyé !');
			}
		}
		return $put;
	}
	...
	public function get($fileFrom, $fileDest){
		...
		if ($this->verboseMode){
			if ($get){
				...
				\Components::Alert('danger', 'Le fichier <code>'.$fileFrom.'</code> n\'a pas pu être téléchargé !');
			}
		}
		return $get;
	}
		...
		if ($this->connectionCheck()) {
			...
			if ($this->verboseMode){
				if ($unlink){
					...
					\Components::Alert('danger', 'le fichier <code>'.$fileToDelete.'</code> n\'a pas pu être supprimé !');
				}
			}
		}
		return $unlink;
